...
such that $f^l \lt g^p \lt f^u$. Therefore, there must exist some integer $q$
...
\begin{theorem}\label{q_convergence}
    \lean{q_convergence}\leanok\uses{sequence_convergence, def:q}
    For any $g \in G$, the sequence $\frac{q_g(n)}{n}$ converges.
\end{theorem}
\begin{proof}\leanok
This follows from \Cref{sequence_convergence} with $C = 1$ after
...
\begin{theorem}\label{phi'_hom}
    \lean{φ'_hom}\leanok\uses{def:phi'}
